...
\begin{abstract}
We extend Soft Concurrent Constraint languages with the possibility to define both local and global knowledge, in the form of variables and constraint associated with them. Being constraints defined as soft, the underlying system of preferences consists in an ordered monoid where it is possible to represent, partially ordered and bipolar (positive/negative) preferences. Such a formalisms  is more general than previous literature and it allows for a \emph{polynomial}  representation of soft constraints. We provide labelled and unlabelled reduction semantics and their correspondence, together with bisimulation equivalences of such languages.
\end{abstract}
...
\emph{Polyadic Soft Language} is \todo{to be introduced}
...
\begin{definition}[families]
	...
	\begin{itemize}
		...
		\item
		\emph{cancellative} if $\forall a, b, c \in A. a \otimes b = a  \otimes c \implies b = c$.
	\end{itemize}
\end{definition}
...
that for any $a, b$ there exists at most a $c$ such that $a b \otimes c$.
It is moreover equivalent to what we could call strongly locality,
that is, $\forall a, b \in A. (a \otimes b) \odiv a = b$. 
...
\begin{proof}[TO BE REDONE]\todo{To be redone}
...
\begin{lemma}
\label{LA2}
Let $\langle A, \tau \rangle \xrightarrow{\alpha} \langle B, \tau' \rangle$ be a reduction 
using the axiom {\bf LA2}. If $\C$ is localised then $\tau' = \alpha \otimes \tau$
and  $\langle A, \sigma \rangle \xrightarrow{\alpha} \langle B, \alpha \otimes \sigma \rangle$
for any $\sigma \leq \tau$.
%
%Moreover, if  $\langle A, \tau \rangle \to \langle B, \tau' \rangle$ then
%$\langle A, \sigma \rangle \to \langle B, \sigma' \rangle$, 
%with $\sigma' = \tau' \otimes (\sigma \odiv \tau)$.
\end{lemma}
\begin{proof}
For the axiom {\bf LA2} it is obvious, since 
$\alpha \leq c \odiv \tau \leq c \odiv \sigma$,
as well as for the rule {\bf LR1}.
...
using the axiom {\bf LA1}, then $\alpha = \1$.

\begin{lemma}
\label{LA1}
Let $\langle A, \tau \rangle \xrightarrow{\monid} \langle B, \tau' \rangle$ be a reduction 
using the axiom {\bf LA1}. 
If $\C$ is cancellative then there exists $\xi$
such that $\tau' = \xi \otimes \tau$
and 
$\langle A, \sigma \rangle \xrightarrow{\monid} \langle B, \xi \otimes \sigma \rangle$
for any $\sigma$.
\end{lemma}
\begin{proof}
...
as well as for the rule {\bf LR1}.
For {\bf LR2} just note that
since $\C$ is cancellative then for the local update 
it holds $\xi = \exists_x (\sigma_1 \odiv (\pi_0 \otimes \sigma))$.
\end{proof}



We remark that such a $\xi$ is unique, i.e., it is the same for all stores.
...
Let $\langle A, \sigma \rangle \to \langle B, \sigma' \rangle$ be a reduction
and $\sigma \leq \tau$.
%
If $\C$ is invertible and cancellative then 
...
%First of all, note that by Lemma~\ref{minor} $\langle A, \sigma \rangle$ is reachable. So, 
%we proceed by induction on the proof of
Thus, consider the reduction $\langle A, \sigma \rangle \to \langle B, \sigma' \rangle$. 

The result is immediate for axiom ${\bf A3}$, with 
$\alpha = \monid$, as well as for 
${\bf R1}$.

Consider ${\bf A1}$ and 
$\langle \hbox{\tell}(c) \mapsto A, \sigma \rangle \xrightarrow{} \langle A, c \otimes \sigma \rangle$.
Again, $\alpha = \1$
and by localisation $\sigma \odiv \tau \leq \1 \leq \alpha$.
Since $\sigma' = c \otimes \sigma$ and $\tau' = c \otimes \tau$,
by invertibility $\sigma' = \tau' \otimes (\sigma \odiv (\tau \otimes \alpha))$.

As for ${\bf A2}$, we have 
$\langle \hbox{\ask}(c) \mapsto A, \sigma \rangle \xrightarrow{} \langle A, \sigma \rangle$ 
with $\sigma \leq c$ and thus
$\langle \hbox{\ask}(c) \mapsto A, \tau \rangle \xrightarrow{\alpha} \langle A, \alpha \otimes \tau \rangle$ 
for any $\alpha \leq c\odiv \tau$.
%
Now $\sigma \leq c$
...
%noting that they are both reachable, the former by Lemma~\ref{minor}.
%we have that 
...
$$\langle A, \pi_0 \otimes \tau \rangle  \xrightarrow{\beta}
  ...
  $${\displaystyle\langle \exists^{\pi}_x A,\tau\rangle \xrightarrow{\beta} 
   \langle \exists^{\pi \chi}_x B, \beta \otimes  \tau \otimes \exists_x \chi
    \rangle \text{ with } \chi = \tau_1 \odiv (\beta \otimes \pi_0 \otimes \tau)}$$
and it suffices to prove that $\sigma \odiv \tau \leq \beta$ and $\chi = \rho$, 
since from these properties and invertibility it follows that
 $\sigma' = \sigma \otimes \exists_x \rho = \beta \otimes \tau \otimes \exists_x \chi \otimes (\sigma \odiv (\tau \otimes \beta))) = 
...
%Further, we already shown that $((a \otimes b) \odiv b) \otimes b = a \otimes b$, which implies
%that  $((a \odiv b) \otimes b) \odiv b = a \odiv b$. From the latter is follows that
%\xi = \tau_1 \odiv (\beta \otimes \pi_\otimes \otimes \tau _0)}
...
%strengthening what is already specified in Lemma~\ref{l-mono}.
hence $\rho = \chi = \1$, and we are done.

If it is \mbox{\bf A1}, that is \mbox{\bf L1} for the labelled reduction, we have that 
$\beta = \1$. 
%
We then have to show the equivalence of $\rho =  \sigma_1 \odiv (\pi_0 \otimes \sigma)$ and
$\chi = \tau_1 \odiv (\pi_0 \otimes \tau)$.
%
Since $\C$ is cancellative by Lemma~\ref{LA1}
we have that $\sigma_1 = \chi \otimes \pi_0 \otimes \sigma$ 
and $\tau_1 = \chi \otimes \pi_0 \otimes \tau$,
and we are done.
%
\comment{
By induction hypothesis 
$\sigma_1 = \tau_1 \otimes ((\pi_0 \otimes \sigma) \odiv (\pi_0 \otimes \tau))
=  (\tau_1 \odiv ( \pi_0 \otimes \tau)) \otimes \pi_0 \otimes \sigma$,
the latter equality proved by invertibility and Lemma~\ref{l-mono}, and thus $\xi \leq \rho$.
Instead, $\rho \leq \xi$ is equivalent to 
 $(\sigma_1 \odiv (\pi_0 \otimes \sigma)) \otimes (\pi_0 \otimes \tau) \leq \tau_1$,
 which by invertibility and Lemma~\ref{mono} is equivalent to 
$\sigma_1 \otimes ((\pi_0 \otimes \tau) \odiv (\pi_0 \otimes \sigma))  \leq \tau_1$,
}
%
\comment{
...
$\langle A, \alpha \otimes  \sigma \otimes d \rangle  \xrightarrow{}\langle A', \sigma' \rangle$.
  By completeness $\langle A,  \sigma \rangle  \xrightarrow{\beta}\langle A', \sigma'' \rangle$,
