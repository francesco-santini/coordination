...
\begin{example}[Power set]\label{ex:powerset}
	Given a (possibly infinite) set $V$ of variables, a key example
	is represented by the POM $\mathbb{P}(V) = \langle 2^V, \subseteq, \cap, V \rangle$
	of subsets of $V$, with the partial order given by subset inclusion
	and the monoidal operator by intersection. 
	In fact, $P(V)$ is a continuous lattice, since all LUBSs exists.
...
\begin{example}
Consider again the SLM $\mathbb{P}(V)$ from Example~\ref{ex:powerset}. 
It is clearly residuated, with $X \odiv Y = (V \setminus Y) \cup X$. In fact, the 
residuated operator plays the role of classical logical implication $Y \implies X$.
Note also that $V \setminus Y = \emptyset \odiv Y$, so algebraically we have 
that $X \odiv Y = X \vee (\bot \odiv Y)$ holds in $\mathbb{P}(V)$.
\end{example}
...
$\sigma \mid_{X}: X \rightarrow V$ the restriction of $\sigma$ to $X$
and by $\sigma^{c}(X) = \{ y \mid \sigma(y) \in X\}$ the counter-image 
of $X$ along $\sigma$.
...
Then, for any possible move of $B$ we have that 
...
%
%In this section we present an example where we show an application of the language to a bipolar example.
%
%Bipolarity is a hot topic in a variety of fields, including psychology, multi-criteria decision making, database queries, argumentation, and qualitative reasoning, to name a few. Positive and negative statements are frequently used to express preferences on a range of options. In fact, in many real-life situations, agents express both their likes and dislikes, resulting in bipolar preferences. Positive and negative preferences may appear to be two distinct criteria to reason with, and so techniques like multi-criteria optimisation may appear to be appropriate for dealing with both. However, this explanation obscures the essential character of bipolar preferences, namely, that positive and negative desires are inherently opposed. For example, the advantages and disadvantages of a decision, which may be viewed as positive and negative preferences, are diametrically opposed aspects, thus it would be unreasonable to analyse them individually, as is typically done in a multi-criteria approach.
%
%One of the basic components of decision-making processes is comparing positive and negative aspects and compensating for their strength, and it is definitely a method generally used to address many real challenges.
