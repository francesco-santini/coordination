...
\begin{table}  %\hfil5
  ...
  \\
    ...
    \exists^{\sigma_1 \odiv \sigma_0}_x B, \sigma \otimes \exists_x(\sigma_1 \odiv (\rho \otimes \sigma _0)) 
    \rangle} \text{ for } x \not \in sv(\sigma)$
    &\bf{Hide}&
  \end{tabular}
  \end{center}
...
Let us discuss in some details axiom {\bf A4}.
The intuition is that if we reach an agent $\langle \exists^{\rho}_x A,\sigma\rangle$, then during the computation
$\exists_x \rho$ has been added to the store. In order to evaluate $A$, the chosen store is 
$\rho \otimes \sigma_0$ for $\sigma_0 = \sigma \odiv \exists_x \rho$, thus removing $\exists_x \rho$ 
and adding $\rho$, which may carry information about $x$. Now, $\sigma_1 \odiv (\rho \otimes \sigma _0)$ 
is precisely the information added by the step originating from $A$, which is then restricted and added to $\sigma$. 
The effect on the local store is instead cumulative, and it is obtained by $\sigma_1 \odiv \sigma_0$. 
...
\begin{remark}
Note that the choice for the resulting global store could have been 
$\sigma_0 \otimes \exists_x(\sigma_1 \odiv \sigma _0)$, which has 
a definitively nicer appearance. Indeed, thanks to Lemma~\ref{mono},
...
\begin{remark}
Note that another choice for the resulting local store could have been 
$\rho_0 = \rho \otimes (\sigma_1 \odiv (\rho \otimes \sigma _0))$. 
However, note that $\rho_0 \leq \sigma_1 \odiv \sigma_0$, and it is a more faithful 
description of the effect of the steps of the computation on the local store. 

Indeed, let us consider the agent $A = \hbox{\tell}(c) \parallel \hbox{\tell}(d)$ and the 
configuration $\langle \exists^\monid A, \sigma \rangle$.
Assuming that $c$ is added to the store, with both choices we end up with the reduction
$\langle \exists^\monid A, \sigma \rangle \mapsto \langle \exists^{\rho'} A, \sigma' \rangle$
such that $\sigma' = \sigma \otimes \exists_x ((c \otimes \sigma) \odiv \sigma)) = \sigma \otimes \exists_x c$, 
since $x \not \in sv(\sigma)$ and $a \otimes ((a \otimes b) \odiv a) = a \otimes b$, and
$\rho' = (c \otimes \sigma) \odiv \sigma$, since the initial local store is $\monid$.
%
...
Now, we need to evaluate $\hbox{\tell}(d)$ in $c \otimes \sigma_0$, with 
$\sigma_0 = ((\sigma \otimes \exists_x c) \odiv \exists_x c)$, resulting in $d \otimes c \otimes \sigma_0$.
%
Going back to the configuration 
$\langle \exists_x^c \hbox{\tell}(d), \sigma \otimes \exists_x c\rangle$, it evolves to a global store
$\sigma \otimes \exists_x c \otimes \exists_x ((d \otimes c \otimes \sigma_0) \odiv (c \otimes \sigma_0))$
\\

DA FINIRE. MA E' UN BORDELLO.
E FORSE $\rho_0$ E' LA SCELTA GIUSTA
\end{remark}
