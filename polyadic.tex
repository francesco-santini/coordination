...
\begin{remark}\label{rmk:soft}
	Some well-known and often used instances of soft constraints are the 
	%\emph{Boolean} ($\langle \{\mathit{false},\mathit{true}\}, \mathit{false} \leq \mathit{true}, \wedge, \mathit{false}, \mathit{true}\rangle$), 
	\emph{Fuzzy} ($\langle [0,1], \leq, \min, 1 \rangle$), \emph{Probabilistic} ($\langle [0,1], \leq, \times, 1 \rangle$), 
...
\begin{definition}[pomonoid action]\label{def:pomo}
...
the interaction of polyadic structures  with residuation. 
%
...
The properties of the pomonoid action (see Definition~\ref{def:pomo})
...
Let\todo{da rivedere} us now move to the polyadic laws (see Definition~\ref{def:poly}).We just consider the third item, 
...
We consider here the extended agent $\exists^{\rho}_x A$, where $\rho$ is meant to represent a local store. More precisely, the extended agent carries some information about the hidden variable $x$ in an incremental way. In the following, we often write $\exists_x A$ for $\exists^{\monid}_x A$.
...
and $fv(\exists^{\rho}_x A) = (fv(A) \cup sv(\rho)) \setminus \{x\}$.
%
...
\begin{itemize}
	...
	\item $(A_1 \parallel A_2)[^y/_x]  = (A_1[^y/_x] \parallel A_2[^y/_x])$
        \item $(\exists^{\rho}_w A)[^y/_x] = \begin{cases} 
			\exists^{(s_{[^y/_x]} \rho)}_w A[^y/_x]  & \text{if } w \not \in \{x, y\} \\
             (\exists^{(s_{[^z/_w]} \rho)}_z A[^z/_w])[^y/_x] & \text{for } z \not \in fv(\exists^{\rho}_w A) \text{ otherwise}
        \end{cases}$
\end{itemize}
\end{definition}
...
$\exists_w^\rho A = \exists_z^{(s_{[^z/_w]}\rho)} A[^z/_w]$ for $z \not \in fv(\exists^{\rho}_w A)$.
...
\begin{table}  %\hfil5
  ...
  \\
    ...
    \mapsto \langle B, \sigma_1 \rangle \text{ with } \sigma_0 = \sigma \odiv \exists_x \rho }
    {\displaystyle\langle \exists^{\rho}_x A,\sigma\rangle \mapsto \langle 
    \exists^{\rho \otimes \rho_0}_x B, \sigma \otimes \exists_x \rho_0
    \rangle \text{ with } \rho_0 = \sigma_1 \odiv (\rho \otimes \sigma _0)} \text{ for } x \not \in sv(\sigma)$
    &\bf{Hide}&
  \end{tabular}
  \end{center}
...
\begin{table}  %\hfil5
  ...
  & \mbox{   }&\mbox{   }&
  \\
    \mbox{\bf R2}& $\frac {\displaystyle \langle A,\sigma \rangle \mapsto \langle A', \sigma' \rangle} 
    {\displaystyle 
    	\begin{array}{l} \langle B\parallel A, \sigma \rangle \mapsto \langle B\parallel A', \sigma' \rangle
    	\end{array}}$& \bf{Par2}&
  \end{tabular}
  \end{center}
...
Let us discuss instead in some details the more complex axiom {\bf A4}.
The intuition is that if we reach an agent $\langle \exists^{\rho}_x A,\sigma\rangle$, then during the computation
at least $\exists_x \rho$ has been added to the global store by the local agent. In order to evaluate $A$, the chosen 
store is 
$\rho \otimes \sigma_0$ for $\sigma_0 = \sigma \odiv \exists_x \rho$, thus removing $\exists_x \rho$ 
and adding $\rho$, which may carry information about $x$. Now, $\rho_0 = \sigma_1 \odiv (\rho \otimes \sigma _0)$ 
is precisely the information added by the step originating from $A$, which is then restricted and added to $\sigma$. 
The same effect on the local store is instead cumulative, and it is obtained by $\rho \otimes \rho_0$. 
...
Then 
\begin{itemize}
\item $\sigma' = (\sigma' \odiv \sigma) \otimes \sigma$
\item $fv(\langle B, \sigma' \rangle) \subseteq fv(\langle A, \sigma \rangle$
\end{itemize}
\end{lemma}
 
Note that item $1$ is derived by the weaker statement $\exists \sigma''.\ \sigma' = \sigma \otimes \sigma''$,
whose proof is immediate, and the properties of the residuation operator.

\begin{remark}
The choice for the resulting local and global store could have been 
$\sigma_1 \odiv \sigma _0$ and $\sigma_0 \otimes \exists_x(\sigma_1 \odiv \sigma _0)$, 
respectively. 
Indeed, these expressions have a nicer appearance,
%Indeed, thanks to Lemma~\ref{mono},
...
$\langle \exists_x^\monid A, \monid \rangle$.
Assuming that $c$ is added to the store, with both choices we end up with the reduction$\gamma \rightarrow \langle \exists_x^c \hbox{\tell}(d), \monid \rangle$.
Now note that $(d \otimes c) \odiv \monid = c \otimes ((d \otimes c) \odiv (c \otimes \monid)) = d \otimes c = \bot$, hence
the choice of the definition for the local store is immaterial. 
With the alternative definition for the global store, we would then reach 
 $\langle \exists_x^c \hbox{\tell}(d), \monid \rangle \rightarrow \langle \exists_x^{\bot} \hbox{\ostop}, \bot \rangle$,
 ...
 $\langle \exists_x^c \hbox{\tell}(d), \monid \rangle \rightarrow \langle \exists_x^{\bot} \hbox{\ostop}, \exists_x (\bot \odiv c) \rangle$, 
...
but going back to our running example, with $c = x \leq 1$ we have $\bot \odiv c = \emptyset \odiv x \leq 1 = x > 1$
and thus $\exists_x (\bot \odiv c) = \monid$.

Let us now consider the alternative choice for the local store. First of all, note that under Lemma~\ref{mono}
we have that $\rho \otimes (\sigma_1 \odiv (\rho \otimes \sigma _0)) \leq \sigma_1 \odiv \sigma_0$:
the smaller term represents a better approximation of the intuitive local store.
%
Indeed, let us consider again the agent $A = \hbox{\tell}(c) \parallel \hbox{\tell}(d)$ 
with no hypothesis %$\exists_x c = \exists_x d = \monid$ 
and the 
configuration $\langle \exists^\monid A, \sigma \rangle$. After both commands are fired, the
local store should intuitively be (the best approximation of)  $c \otimes d$.
Assuming that $c$ is added to the store, with both choices of the definition for the local store we end up with the reduction
$\langle \exists^\monid A, \sigma \rangle \mapsto \langle \exists^{\rho'} \hbox{\tell}(d), \sigma' \rangle$
such that $\rho' = (c \otimes \sigma) \odiv \sigma$, since the initial local store is $\monid$,
and $\sigma' = \sigma \otimes \exists_x ((c \otimes \sigma) \odiv \sigma)) = \sigma \otimes \exists_x c$, 
since $x \not \in sv(\sigma)$. %and $a \otimes ((a \otimes b) \odiv a) = a \otimes b$, 
%
%Note that in general $c \leq \rho'$, yet let us assume that for simplicity the equality hold, e.g. because we are in 
%an invertible semiring and $c \leq 1$.
%
Now, we need to evaluate 
%$\hbox{\tell}(d)$ in $\rho' \otimes (\sigma' \odiv \rho')$, obtaining 
%$d \otimes \rho' \otimes (\sigma' \odiv \rho')$.
%
% with 
%$\sigma'' = ((\sigma \otimes \exists_x c) \odiv \exists_x c)$, resulting in $d \otimes \rho' \otimes \sigma''$.
%
%Going back to 
the configuration 
$\langle \exists_x^{\rho'} \hbox{\tell}(d), \sigma'\rangle$, which we already argued must evolve to a global store
$\sigma \otimes \exists_x c \otimes \exists_x ((d \otimes \rho' \otimes \sigma'') \odiv (\rho' \otimes \sigma''))$
for $\sigma'' =  \sigma' \odiv \exists_x \rho'$. Now we have that
$c \otimes d \leq \rho' \otimes ((d \otimes \rho' \otimes \sigma'') \odiv (\rho' \otimes \sigma''))
\leq (d \otimes \rho' \otimes \sigma'') \odiv \sigma''$, 
hence confirming our choice of the definition for the local store.
\end{remark}
...
and they are also congruences. Indeed, a context can modify the behaviour of a configuration only by adding constraints to its store. \todo{CHECK!!!}
%\marginpar{to be proved for $\exists_x-$ (not needed for Prop.3)}


\section{Labelled transition system [TO BE DONE]}
...
\begin{definition}[Labelled reductions]
	Let $\Gamma = {\mathcal A} \times \C$ be the set of \emph{configurations} and $V$ the set of variables.
	...
	$\langle \mathcal{C}, 2^V \rangle$,
	%$2^V$,
	%between them, 
	i.e., $\mapsto = \bigcup_{\alpha \in \mathcal{C}, \Delta \subseteq V} \xmapsto{\, A \,}$ and 
	$\xmapsto{\,A \,} \, \, \subseteq \, \,\Gamma \times \Gamma$, obtained by the rules in 
	Table~\ref{fig:ALTS}.
	...
	$\langle \mathcal{C}, 2^V \rangle$;
	$\xrightarrow{\,A\,} \, \, \subseteq \, \,\Gamma \times \Gamma$ can obtained by the rules in 
...
$\xmapsto{\;  \alpha \;}a \, \, \subseteq \, \,\Gamma \times \Gamma$ indexed over 
$\langle \mathcal{C}, 2^V \rangle$;
...
from $\langle A, \sigma\rangle$  to $\langle A', \sigma' \rangle$, i.e., $\langle A, \sigma \otimes \alpha\rangle \longrightarrow \langle A' , \sigma' \rangle$.
...
\begin{table}  %\hfil5
   ...
   \begin{tabular}{llll} 
   %
   \mbox{\bf LA1}& ${\displaystyle \langle \hbox{\tell}(c), \sigma \rangle \xmapsto{\;  \bot \;}
   \langle \hbox{\ostop}, \sigma \otimes c\rangle}$
   \ \ \ & \bf{Tell}&
  \\ 
  &\mbox{   }&\mbox{   } &\mbox{   }
  \\
  \mbox{\bf LA2}& ${\displaystyle
  	\begin{array}{l} \langle \hbox{\ask}(c) \rightarrow A, \sigma \rangle \xmapsto{c  \odivv  \sigma}
  ...
  {\displaystyle\langle p(y),\sigma\rangle \xmapsto{\bot} \langle  A[^y/_x], \sigma\rangle}$ 
  &\bf{Rec}&
    \\ \\
  \mbox{\bf LA4}& $\frac {\displaystyle \langle A, c \otimes d_0 \rangle \xmapsto{\alpha}
  \langle B, d_1 \rangle \wedge d_0 = (d \otimes \alpha) \odiv \exists_x c \wedge x \not \in sv(d) \cup sv(c) \wedge c_1 = d_1 \odiv d_0 }
  {\displaystyle \langle \exists^c_x A, d \rangle \xmapsto{\alpha} 
  \langle \exists^{c_1}_x B, d_0 \otimes \exists_x c_1 \rangle}$
&\bf{Hide}&
...
\begin{table}  %\hfil5
  ...
  & \mbox{   }&\mbox{   }&
  \\
    \mbox{\bf LR2}& $\frac {\displaystyle \langle A,\sigma \rangle \xrightarrow{\alpha} \langle A', \sigma'   \rangle
    	\wedge fv(B) \subseteq \Delta} 
    {\displaystyle 
    	\begin{array}{l} \langle B\parallel A, \sigma \rangle \xrightarrow{\alpha} \langle B\parallel A', \sigma' \rangle
    	\end{array}}$& \bf{Par2}&
    \\
    &\mbox{   }&\mbox{   }&
  \end{tabular}
...
$\langle A, a \rangle \xmapsto{c} \langle B, b \rangle \implies \langle A, a \otimes c \rangle
\mapsto \langle B, b \rangle$
\end{theorem}

\begin{proof}
Tell and Rec \\
\[\sigma \otimes \bot = \sigma \]
\[\langle \tell(c), \sigma \rangle \xmapsto{\bot} \langle \ostop, \sigma \rangle \implies \]
\[\langle \tell(c), \sigma \otimes \bot \rangle = \langle \tell(c), \sigma \rangle 
\mapsto \langle stop, \sigma \rangle\]
\[\langle p(y), \sigma \rangle \xmapsto{\bot} \langle A[^y/_x], \sigma \rangle \implies \]
\[\langle p(y), \sigma \otimes \bot \rangle = \langle p(y), \sigma \rangle \mapsto
\langle A[^y/_x], \sigma \]
Ask \\
\[\langle \ask(c) \rightarrow A, \sigma \rangle \xmapsto{c \odiv \sigma}
\langle A, \sigma \otimes (c \odiv \sigma) \rangle \implies \]
NB $\sigma \otimes (c \odiv \sigma) \leq c $
\[\langle \ask(c) \rightarrow A, \sigma \otimes (c \odiv \sigma) \rangle \mapsto
\langle A, \sigma \otimes (c \odiv \sigma) \rangle \]
Hide \\
hyp: $\langle A, c \otimes d_0 \rangle \xmapsto{\alpha} \langle B, d_1 \rangle \implies
\langle A, c \otimes d_0 \otimes \alpha \rangle \mapsto \langle B, d_1 \rangle$ \\
$d_0 = (d \otimes \alpha) \odiv \exists_x c$ \\
$c_1 = d_1 \odiv d_0$ \\
\[\langle \exists^c_x A, d \rangle \xmapsto{\alpha} \langle 
\exists^{d_1 \odiv ((d \otimes \alpha) \odiv \exists_x c)} B, 
((d \otimes \alpha) \odiv \exists_x c) \otimes 
\exists_x (d_1 \otimes ((d \otimes \alpha) \odiv \exists_x c)) \implies\]
\[\langle \exists^c_x A, d \otimes \alpha \rangle \mapsto \langle
\exists^{d_1 \odiv ((d \otimes \alpha) \odiv \exists_x c)} B,
((d \otimes \alpha) \odiv \exists_x c) \otimes 
\exists_x (d_1 \otimes ((d \otimes \alpha) \odiv \exists_x c))\]
\end{proof}

\begin{theorem}[Completeness]
$c \leq a$?, $\langle A, a \rangle \mapsto \langle B, b \rangle \implies 
\langle A, c \rangle \xmapsto{\alpha} \langle B, d \rangle$ with 
$c \otimes \alpha \otimes \beta = a \wedge d \otimes \beta = b$ for some $\beta$.
\end{theorem}

\begin{proof}
Tell and Rec \\
\[ \langle \tell(c), \sigma \rangle \mapsto \langle \ostop, \sigma \rangle \wedge (c \leq a) \]
\[ \langle \tell(c), \sigma' \rangle \xmapsto{\bot} \langle \ostop, \sigma' \rangle \]
\[ \implies \beta = \sigma \odiv \sigma' = \bigvee\{c \mid \sigma' \otimes c \leq \sigma\} \]
\end{proof}
