...
$\sigma = \rho$. This fact does not hold if we move to weak relations. To this end, 
...
and they are also congruences. Indeed, a context $C[\cdot]$, i.e., an agent with an placeholder $\cdot$,
...
then $\langle A,\sigma \otimes d\rangle \sim_{\mathit{s}} \langle B,\rho \otimes d \rangle$
...
Although $\sim_{\mathit{s}}$ and $\approx_{\mathit{s}}$ are fully abstract, it is somewhat unsatisfactory
...
\label{l-mono}
Let $\langle A, \sigma \rangle \xrightarrow{\alpha} \langle B, \rho \rangle$ be a reduction. 
Then 
%\begin{enumerate}
...
%
For the latter part, note that $\alpha \neq 1$ just means that the rules {\bf LA1}
...
%\end{enumerate}
$\sigma = (\rho \odiv (\exists_x \pi)_\otimes) \otimes (\exists_x \pi)_\otimes$.
\end{lemma}
...
\begin{enumerate}
\item if $\langle A, \sigma \rangle \downarrow_c$ then $\langle B, \rho \rangle \downarrow_c$;
\item if $\langle A, \sigma \rangle \to \gamma_1$ then there is $\gamma_2$ such that $\langle B, \alpha \otimes \rho \rangle \to \gamma_2$ 
...
$\sigma = \rho$, as for the saturated bisimilarity.
...
\begin{enumerate}
\item if $\langle A, \sigma \rangle \downarrow_c$ then $\langle B, \rho \rangle \Downarrow_c$;
\item if $\langle A, \sigma \rangle \to \gamma_1$ then there is $\gamma_2$ such that $\langle B, \rho \otimes \alpha \rangle \Rightarrow \gamma_2$ 
and $(\gamma_1, \gamma_2) \in R$;
\end{enumerate}
...
We have now the ingredients to prove key results for strong and weak bisimilarity.

\begin{proposition}
Let $\langle A,\sigma \rangle, \langle B,\rho \rangle$ be configurations, $d \in \mathcal{C}$
and $C[\cdot]$ a context. Moreover, let $\mathcal{C}$ be invertible.
If $\langle A,\sigma \rangle \sim \langle B,\rho \rangle$
then $\langle A,\sigma \otimes d\rangle \sim \langle B,\rho \otimes d \rangle$
and $\langle C[A],\sigma \rangle  \sim \langle C[B],\rho \rangle$.
%
%Moreover, the same property holds for $\approx$.
\end{proposition}
\begin{proof}
XXXX
\end{proof}
...
and $C[\cdot]$ a context. Moreover, let $\mathcal{C}$ be invertible.
If $\langle A,\sigma \rangle \approx \langle B,\rho \rangle$
then $\langle A,\sigma \otimes d\rangle \approx \langle B,\rho \otimes d \rangle$
and $\langle C[A],\sigma \rangle  \approx \langle C[B],\rho \rangle$.
%
%Moreover, the same property holds for $\approx$.
\end{proposition}
\begin{proof}
