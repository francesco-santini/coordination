...
then $\langle A, \sigma \rangle$ is reachable from
 $\langle \hbox{\tell}(\sigma \odiv \tau) \parallel B, \mu \rangle$ and
\end{proof}
...
\end{proposition}
\begin{proof}
Let $\langle A,\sigma \otimes d\rangle \downarrow_c$, that is, \sigma \otimes d\rangle \leq c$.
Since \langle A, \sigma \rangle \approx \langle b, \rho \rangle$ and $\langle A,\sigma \otimes d\rangle \downarrow_\sigma$,
...
B?, ???and ??????for ?=fv(
B?, ???) \fv(
B, ??). Let us assume, without loss of generality, that ??sv(d) =?; since reductions are monotone (item 4 of Lemma6), we have 
B, ??d? ??
B?, ???d?. Finally, c???d =????d ????????d ???(???d), hence 
B, ??d? ?c.
