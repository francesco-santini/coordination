...
and we denote as $\mapsto^\ast$ the reflexive and transitive closure of $\mapsto$.

\begin{lemma}[On monotonicity, II]
\label{mono2}
Let $A$ be an initial agent and 
$\langle A, \sigma \rangle \mapsto^\ast \langle B \parallel \exists_x^\pi C, \sigma' \rangle$ 
...
\label{l-mono2}
Let $A$ be an initial agent and 
$\langle A, \sigma \rangle \xRightarrow{\mu} \langle B \parallel \exists_x^\pi C, \sigma' \rangle$ 
a sequence of reductions. 
Then 
\begin{enumerate}
\item $\exists \sigma''.\  \sigma' = \bigotimes \mu \otimes \sigma'' \otimes \exists_x \pi$
\end{enumerate}
\end{lemma}
...
Let $\langle A, \sigma \rangle$ be a l-reachable configuration and
...
Before proceeding to completeness, note that if a configuration $\langle A, \sigma \rangle$ is reachable,
then it is also obviously l-reachable via a sequence of reductions labelled by $\monid$.

\begin{theorem}[Completeness]
Let $\langle A, \sigma \rangle$, $\langle A, \tau \rangle$ be reachable configurations such that
$\sigma \leq \tau$ and
$\langle A, \sigma \rangle \mapsto \langle B, \sigma' \rangle$ a reduction. 
Then $\langle A, \tau \rangle$ is a l-reachable configuration and
$\langle A, \tau \rangle \xrightarrow{\alpha} \langle B, \tau'' \rangle$
for some $\alpha$ such that $\sigma = \tau \otimes \alpha \otimes \sigma''$ and 
$\sigma' = \tau' \otimes \sigma''$ for some $\sigma''$.
\end{theorem}
...
$(\sigma \odiv \tau) \otimes \tau \leq (\sigma \odiv \tau) \otimes \tau$, which clearly holds.


Since $\sigma \leq c$, by invertibility $\sigma = (\sigma \odiv c) \otimes c \leq (c \odiv \tau) \otimes \tau$
[TO BE FINISHED]
