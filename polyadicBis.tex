...
that the parallel operator is associative and commutative, as well as $ \hbox{\ostop} \parallel A = A$.
%
%
...
and a configuration initial if so is its associated agent and reachable if it can be
reached by an initial agent via a sequence of reductions.
...
since $\alpha$ is present in the global store, it will not be necessary to receive it again in the future. 
...
$$\langle A, \bigotimes \pi \otimes \widehat{\sigma_0} \rangle \mapsto
 ...
 with $\widehat{\sigma_0} = (\alpha \otimes \sigma) \odiv \bigotimes \exists_x \pi$
...
for $3$ we used the well-known law $a \otimes ((a \otimes b) \odiv a) = a \otimes b$.
\qed
\end{proof}
...
The proof simply consists in adding in parallel the agent $\hbox{\ask}(\sigma \odiv \tau) \rightarrow \mathit{\ostop}$.

\begin{lemma}
\label{riminor}
Let $\langle A, \tau \rangle \xrightarrow{\beta} \langle B, \tau' \rangle$ be a reduction
such that $\alpha \leq \beta$. Then $\langle A, \tau \rangle \xrightarrow{\alpha} \langle B, \tau' \otimes (\alpha \odiv \beta) \rangle$.
\end{lemma}

The lemma just states that the label in a reduction can always be strengthened.
%and it exploits the premise of rule {\bf LA2} together with Lemma~\ref{l-mono}.
...
As for ${\bf LA2}$, note that $\sigma' = \sigma$, so we need
to prove that $\langle \hbox{\ask}(c) \rightarrow A, \tau \rangle \xrightarrow{\alpha} \langle A, \alpha \otimes \tau \rangle$ and $\sigma = \alpha \otimes \tau \otimes \sigma''$ for some $\alpha$ and $\sigma''$.
A possible candidate for $\alpha$ is $c \odiv \tau$, since it clearly makes the reduction possible,
while for $\sigma''$ is instead $\sigma \odiv (\alpha \otimes \tau)$: should $\sigma \leq \alpha \otimes \tau$
then the result would follow by invertibility. Again by invertibility, the latter corresponds to prove that 
$(\sigma \odiv \tau) \otimes \tau \leq (c \odiv \tau) \otimes \tau$, which holds since $\sigma \leq c$.
...
$$\langle \exists^\pi_x A, \sigma \rangle \mapsto
  ...
  with $\sigma_0 = \sigma \odiv \exists_x \pi$, $\rho = \sigma_1 \odiv (\bigotimes \pi  \otimes \sigma_0)$.
...
$$\langle A, \bigotimes \pi \otimes \tau_0 \rangle  \xrightarrow{\beta}
  ...
  for some $\sigma_2$.
Moreover, $\tau_1 = (\tau_0 \odiv (\beta \otimes \bigotimes \pi \otimes \tau_0 )) \otimes  \beta \otimes \bigotimes \pi \otimes \tau_0$
by Lemma~\ref{l-mono}.
