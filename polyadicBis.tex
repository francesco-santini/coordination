...
%
It holds for initial configurations, and it is clearly preserved by reductions, 
...
then $\langle C[A],\sigma \otimes d\rangle \sim_{\mathit{s}} \langle C[B],\rho \otimes d \rangle$.
%and $\langle C[A],\sigma \rangle  \sim_{\mathit{s}} \langle C[B],\rho \rangle$.
%
Moreover, the same property holds for $\approx_{\mathit{s}}$.
\end{proposition}
\begin{proof}
By definition $\sim_{\mathit{s}}$ and $\approx_{\mathit{s}}$ are upward closed, so we just need to check congruence.
...
\section{On the correspondence between reduction semantics [RIVEDERE DA QUA]}
\label{corres}
...
\begin{lemma}
\label{riminor}
Let $\langle A, \tau \rangle \xrightarrow{\beta} \langle B, \tau' \rangle$ with
$\alpha \leq \beta$. 
%
If $\beta \neq \monid$ then $\langle A, \tau \rangle \xrightarrow{\alpha} \langle B, \tau' \otimes (\alpha \odiv \beta) \rangle$.
%\\
...
\section{Labelled bisimulation [RIVEDERE DA QUA]}
...
The feasibility of $\sim$ and $\approx$ resides on the fact that they are not 
upward closed by definition. Thanks to the correspondence results in Section~\ref{corres}, 
they can be proved so, as well as congruences.
%
