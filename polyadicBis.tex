...
s\begin{example}[Power set]\label{ex:powerset}
...
a residuated POM $\mathbb S = \langle {\mathcal C}, \leq, \otimes, \odiv, \1\rangle$, which is
...
It induces an operator $[^y/_x]: \mathcal{A} \rarrow \mathcal{A}$ on agents in the expected way,  in particular
...
Then $\sigma' = (\sigma' \odiv \sigma) \otimes \sigma$ and $fv(\langle B, \sigma' \rangle) \subseteq fv(\langle A, \sigma \rangle)$.
%\begin{enumerate}
...
\begin{proof}
The first law is due to the equivalence $((a \otimes b) \odiv b) \otimes b = a \otimes b$, which holds in all residuated POMs.
The second law is by induction on rule application.
\end{proof}
...
\begin{definition}
An agent $A$ is initial if it contains only existential operators of the form $\exists_x B$.
A configuration is initial if so is its associated agent and reachable if it can be
reached by an initial configuration via a sequence of reductions.
\end{definition}
...
\item if $\langle A, \sigma \rangle \to \gamma'_1$ then there is $\gamma'_2$ such that $\langle B, \rho \rangle \to \gamma'_2$ and $(\gamma'_1, \gamma'_2) \in R$;
...
We now let $\rightarrow$ denote the reflexive and transitive closure of $\to$.
...
\begin{enumerate}
\item if $\langle A, \sigma \rangle \downarrow_c$ then $\langle B, \rho \rangle \Downarrow_c$;
\item if $\langle A, \sigma \rangle \to \gamma'_1$ then there is $\gamma'_2$ such that $\langle B, \rho \rangle \Rightarrow \gamma'_2$ and $(\gamma'_1, \gamma'_2) \in R$;
\item $(\langle A,\sigma \otimes d\rangle, \langle B,\rho \otimes d \rangle) \in R$ for all $d \in \mathcal{C}$.
\end{enumerate}
...
that is, $\langle \hbox{\ask}(c) \mapsto A, \sigma \rangle \xrightarrow{c \odiv \sigma} \langle A, (c \odiv \sigma) \otimes \sigma \rangle$. However, this might have been restrictive in combination with rule ${\bf LR2}$.
Consider again  our running example and the configuration 
$\langle \exists^{[x > 1]}_x \hbox{\ask}(y > 2) \mapsto \hbox{\ostop}, \monid$. The initial configuration in the premise is
...
$$\langle A, \alpha \otimes \bigotimes \pi \otimes \sigma_0 \rangle \to
  \langle B, \sigma_1 \rangle$$
...
$\langle A, \sigma \rangle \to \langle B, \sigma' \rangle$ a reduction. 
...
Moreover, if  $\langle A, \tau \rangle \to \langle B, \tau' \rangle$ and
$\langle A, \sigma \rangle \to \langle B, \sigma' \rangle$ are
...
$$\langle A, \bigotimes \pi \otimes \sigma_0 \rangle \to
  \langle B, \sigma_1 \rangle$$
%
