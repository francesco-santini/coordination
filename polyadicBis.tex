...
Then it holds
	\begin{itemize}
         \item $\forall a, b \in A.\ \exists_x(a \odiv \exists_x b) \leq \exists_x a \odiv \exists_x b \leq
...
We can now show that $\odiv$ does not substantially alter the free variables of its arguments, namely,
that $sv(a \odiv b) \subseteq sv(a) \cup sv(b)$ holds. Indeed, if $x \not \in sv(a) \cup sv(b)$
\[\exists_x(a \odiv b) =  \exists_x(a \odiv \exists_x b) \leq \exists_x a \odiv \exists_x b \leq
  \exists_x(\exists_a \odiv b) =\exists_x(a \odiv b)\]
...
a CLIM $\mathbb S = \langle {\mathcal C}, \leq, \otimes\rangle$, which is
polyadic over $V$ and whose elements
...
p(x) \mid \exists^{\pi}_x A.\]  
for $\pi \in {\mathcal C}^\ast$ a sequence of elements.
\end{definition}

We consider here the extended agent $\exists^{\pi}_x A$, where $\pi$ is meant to represent the sequence of updates performed on the local store. More precisely, the extended agent may carry some information about the hidden variable 
$x$ in an incremental way. In the following, we often write $\exists_x A$ for $\exists^{[ ]}_x A$ and $\pi_i$ for 
...
It induces a substitution operator $[^y/_x]: \mathcal{A} \rarrow \mathcal{A}$ on agents as follows
...
\begin{itemize}
	...
	\item $(A_1 \parallel A_2)[^y/_x]  = (A_1[^y/_x] \parallel A_2[^y/_x])$
        \item $(\exists^{\pi}_w A)[^y/_x] = \begin{cases} 
			\exists^{(s_{[^y/_x]} \pi)}_w A[^y/_x]  & \text{if } w \not \in \{x, y\} \\
             (\exists^{(s_{[^z/_w]} \pi)}_z A[^z/_w])[^y/_x] & \text{for } z \not \in fv(\exists^{\pi}_w A) \text{ otherwise}
        \end{cases}$
\end{itemize}
with $s_{[^y/_x]} \langle \pi_1, \ldots, \pi_n \rangle = \langle s_{[^y/_x]} \pi_1, \ldots, s_{[^y/_x]} \pi_n \rangle$.
\end{definition}
...
$\bigotimes \pi$ and $\exists_x \pi$
as shorthands for $\pi_1 \otimes \ldots \otimes \pi_n$ and
$(\exists_x \pi_1) \otimes \ldots \otimes (\exists_x \pi_n)$, respectively.
...
such that $\mapsto \, \, \subseteq \, \,\Gamma \times   \Gamma$ is the 
...
\begin{table}  %\hfil5
  ...
  \mbox{\bf A2}& $\frac {\displaystyle \sigma \leq c}{\displaystyle
  	\begin{array}{l} \langle \hbox{\ask}(c) \rightarrow A, \sigma \rangle \mapsto \langle A, \sigma \rangle   	\end{array}}$
    \ \ \ & \bf{Ask}&
    \\
    &\mbox{   }&\mbox{   }&
    \\
  \mbox{\bf A3}& $\frac {\displaystyle p(x) = A \in \mathcal{P} }
  {\displaystyle\langle p(y),\sigma\rangle \mapsto \langle A[^y/_x], \sigma \rangle}$ 
  &\bf{Rec}&
    %\\
    %&\mbox{   }&\mbox{   }&
    %\\
...
\begin{table}  %\hfil5
  %\scalebox{0.9}{
   ...
   \begin{tabular}{lcll} 
   %
  \mbox{\bf R1}& $\frac {\displaystyle \langle A, \sigma\rangle \mapsto \langle A', \sigma' \rangle} 
  {\displaystyle \begin{array}{l}
                          \langle A\parallel B, \sigma \rangle \mapsto \langle A'\parallel B, \sigma' \rangle
                          \end{array}}$ 
    & \bf{Par1}&
  \\
  & \mbox{   }&\mbox{   }&
  \\
    \mbox{\bf R2}& $\frac {\displaystyle \langle A, \bigotimes \pi \otimes \sigma_0 \rangle
    \mapsto \langle B, \sigma_1 \rangle \text{ with } \sigma_0 = \sigma \odiv \exists_x \pi}
    {\displaystyle\langle \exists^{\pi}_x A,\sigma\rangle \mapsto \langle 
    \exists^{\pi \rho}_x B, \sigma \otimes \exists_x \rho
    \rangle \text{ with } \rho = \sigma_1 \odiv (\bigotimes \pi \otimes \sigma _0)} \text{ for } x \not \in sv(\sigma)$
    &\bf{Hide}&
  \end{tabular}
  \end{center}
...
a sequence $\pi$ of updates has been performed by the local agent and $\exists_x \pi$ 
has been added to the global store. In order to evaluate $A$, the chosen 
store is 
$\bigotimes \pi \otimes \sigma_0$ for $\sigma_0 = \sigma \odiv \exists_x \pi$, thus removing 
the effect $\exists_x \pi$ of the sequence of updates from the global store
and adding the cumulative effect $\bigotimes \pi$ of the local store, which may carry information about $x$. 
Now, $\rho = \sigma_1 \odiv (\bigotimes \pi \otimes \sigma _0)$ 
...
\label{rmono}
Let $\langle A, \sigma \rangle \rightarrow \langle B, \sigma' \rangle$ be a reduction. 
Then 
\begin{enumerate}
...
$\gamma \mapsto \langle \exists_x^{[ c ]} \hbox{\tell}(d), \monid \rangle$.
With the alternative definition for the global store, we would then reach 
 $\langle \exists_x^{[c]} \hbox{\tell}(d), \monid \rangle \mapsto \langle \exists_x^{[c,\bot \odiv c]} \hbox{\ostop}, \bot \rangle$,
 ...
 $\langle \exists_x^{[c]} \hbox{\tell}(d), \monid \rangle \mapsto \langle \exists_x^{[c,\bot \odiv c]} \hbox{\ostop}, \exists_x (\bot \odiv c) \rangle$, 
...
We say that an agent $A$ is initial if it contains only existential operators of the form $\exists_x B$
and that a configuration is initial if so is its associated agent and reachable if it can be
reached by an initial comfiguration via a sequence of reductions.

\begin{lemma}[On monotonicity, II]
\label{mono2}
Let $\langle B \parallel \exists_x^\pi A, \sigma \rangle$ 
...
$\sigma \odiv \exists_x \pi$.


\section{Saturated Bisimulation [to be checked]}\label{sec:saturated}
...
\item if $\langle A, \sigma \rangle \mapsto \gamma'_1$ then there is $\gamma'_2$ such that $\langle B, \rho \rangle \mapsto \gamma'_2$ and $(\gamma'_1, \gamma'_2) \in R$;
...
We now let $\mapsto^*$ denote the reflexive and transitive closure of $\mapsto$.
...
$\gamma \mapsto^* \gamma'$ and $\rho \leq c$.
\end{definition}
...
obtained from Definition~\ref{def:strongsb} by replacing $\mapsto$ with $\mapsto^*$ and $\downarrow_c$ with $\Downarrow_c$.
\end{definition}

Since $\sim_{\mathit{s}}$ and $\approx_{\mathit{s}}$ are saturated bisimulations, they are clearly upward closed 
and they are also congruences. Indeed, a context can modify the behaviour of a configuration only by adding constraints to its store. CHECK!!!
...
\begin{definition}[Labelled reductions]
	...
	such that $\mapsto \, \, \subseteq \, \,\Gamma \times \mathcal{C} \times \Gamma$ is the ternary
...
\begin{table}  %\hfil5
  ...
  \mbox{\bf LA2}& $\frac{\displaystyle \alpha \leq  c  \odiv  \sigma} {{\displaystyle
  	\begin{array}{l} \langle \hbox{\ask}(c) \rightarrow A, \sigma \rangle \xrightarrow{\alpha}
...
\begin{table}  %\hfil5
  ...
  & \mbox{   }&\mbox{   }& \mbox{   }
  \\
  \mbox{\bf LR2} & $\frac {\displaystyle \langle A, \bigotimes \pi \otimes \sigma_0 \rangle \xrightarrow{\alpha}
  \langle B, \sigma_1 \rangle \text{ with } \sigma_0 = \sigma \odiv \exists_x \pi }
  {\displaystyle \langle \exists^\pi_x A, \sigma \rangle \xrightarrow{\alpha}
  \langle \exists^{\pi \rho}_x B, \alpha \otimes \sigma \otimes \exists_x \rho \rangle  \text{ with } \rho = \sigma_1 \odiv (\alpha \otimes \bigotimes \pi  \otimes \sigma_0)}
...
For the rules in Table~\ref{fig:ALTS}, as well as for rule {\bf  LR1}, we can restate the intuition given for their unlabelled counterparts. The difference concerns the axioms for the $\hbox{\ask}(c)$: if $c$ is not entailed from $\sigma$, then
...
%
Note that by choosing $\rho = \sigma_1 \odiv (\alpha \otimes \bigotimes \pi  \otimes \sigma_0)$ we are 
...
that is, $\langle \hbox{\ask}(c) \rightarrow A, \sigma \rangle \xrightarrow{c \odiv \sigma} \langle A, (c \odiv \sigma) \otimes \sigma \rangle$. However, this might have been restrictive in combination with rule ${\bf LR2}$.
Consider again  our running example and the configuration 
$\langle \exists^{[x > 1]}_x \hbox{\ask}(y > 2) \rightarrow \hbox{\ostop}, \monid$. The initial configuration in the premise is
$\langle \hbox{\ask}(y > 2) \rightarrow \hbox{\ostop}, x > 1 \rangle$, and $(y > 3) \odiv (x > 1) = (x \leq 1) \vee (y > 2)$.
...
the configuration $\gamma = \langle \exists^{[x > 1]}_x \hbox{\ask}(x > 2) \rightarrow \hbox{\ostop}, \monid \rangle$. 
We would have that 
$\langle \hbox{\ask}(x > 2) \rightarrow \hbox{\ostop}, x > 1 \rangle \xrightarrow{x \neq 2} \langle \hbox{\ostop}, x > 2 \rangle$,
...
Then 
\begin{enumerate}
\item $\exists \sigma''.\  \sigma' = \alpha \otimes \sigma'' \otimes \sigma$ and $\alpha \neq \monid$ implies $\sigma'' = \monid$
\item $fv(\langle B, \sigma' \rangle) \subseteq fv(\langle A, \sigma \rangle) \cup sv(\alpha)$
\end{enumerate}
\end{lemma}

\begin {proof}
We just skecth the proof for the rule {\bf LR2} in item $1$. Clearly we have that 
$\exists_x \rho$  is a candidate for the equality, and we must then show that it is indeed 
a witness.
Note that by induction hypothesis in the premise of the rule 
$\sigma_1 = \alpha \otimes \sigma'' \otimes \bigotimes \pi_i \otimes \sigma_0$
for some $\sigma''$ such that $\alpha \neq \monid$ implies $\sigma'' = \monid$.
In the conclusion we then have $\rho =  (\alpha \otimes \sigma'' \otimes \bigotimes \pi_i \otimes \sigma_0) \odiv 
(\alpha \bigotimes \pi_i \otimes \sigma_0)$. If $\alpha \neq \monid$, then $\rho = \monid$ and we are done.
\qed
\end{proof}
...
$ \langle \exists^\pi_x A, \sigma \rangle \xrightarrow{\alpha}
  ...
  Item $1$ of the lemma above tell us that if $\alpha \neq \monid$, then $\rho = \monid$. 
  Indeed, this is the expected behaviour: if an input from the context is needed,
  there is no contribution from the local agent, hence the update is correctly put to $\monid$.
\end{remark}

We say that a configuration is l-reachable if it can be
reached by an initial configurations via a sequence of 
labelled reductions.
...
\section{Labelled bisimulation [TO BE DONE]}

\section{On the equivalence between reduction semantics}
...
%$\langle A, \sigma \rangle \mapsto^\ast \gamma$, and it is l-reachable if 
...
$\langle A, \sigma \rangle \xrightarrow{\alpha} \langle B, \sigma' \rangle$ a reduction. 
Then $\langle A, \alpha \otimes \sigma \rangle$ is a reachable configuration and
$\langle A, \alpha \otimes \sigma \rangle \mapsto \langle B, \sigma' \rangle$.
\end{theorem}
...
$$\langle \exists^\pi_x A, \sigma \rangle \xrightarrow{\alpha}
  \langle \exists^{\pi \rho}_x B, \alpha \otimes \sigma \otimes \exists_x \rho \rangle$$
  with $\sigma_0 = \sigma \odiv \exists_x \pi$, $\rho = \sigma_1 \odiv (\alpha \otimes \bigotimes \pi  \otimes \sigma_0)$, and 
  $x \not \in sv(\sigma) \cup sv(\alpha)$.
The premise tells us that $\langle A, \bigotimes \pi \otimes \sigma_0 \rangle$ is l-reachable and
$$\langle A, \bigotimes \pi \otimes \sigma_0 \rangle \xrightarrow{\alpha}
...
$$\langle A, \alpha \otimes \bigotimes \pi \otimes \sigma_0 \rangle \mapsto
  \langle B, \sigma_1 \rangle$$
Now recall that we must prove that $\langle \exists^\pi_x A, \alpha \otimes \sigma \rangle$ is reachable and
$$\langle \exists^\pi_x A, \alpha \otimes \sigma \rangle \mapsto
  \langle \exists^{\pi \rho}_x B, \alpha \otimes \sigma \otimes \exists_x \rho \rangle$$
which would be implied by $\langle A, \bigotimes \pi \otimes \widehat{\sigma_0} \rangle$ being reachable and
$$\langle A, \bigotimes \pi \otimes \widehat{\sigma_0} \rangle \mapsto
  \langle B, \widehat{\sigma_1} \rangle$$
 with $\widehat{\sigma_0} = (\alpha \otimes \sigma) \odiv \exists_x \pi$
...
%
Now we have that 
$$\bigotimes \pi \otimes \widehat{\sigma_0} = \bigotimes \pi \otimes ((\alpha \otimes \sigma) \odiv \exists_x \pi)$$ 
$$=^1 \bigotimes \pi \otimes ((\alpha \otimes (\sigma \odiv \exists_x \pi) \otimes \exists_x \pi ) \odiv \exists_x \pi)$$
$$=^2  (\bigotimes \pi \odiv \exists_x \pi) \otimes \exists_x \pi \otimes ((\alpha \otimes (\sigma \odiv \exists_x \pi) \otimes  \exists_x \pi ) \odiv \exists_x \pi)$$
$$ =^3 (\bigotimes \pi \odiv \exists_x \pi) \otimes \alpha \otimes (\sigma \odiv \exists_x \pi) \otimes \exists_x \pi$$
$$= \alpha \otimes \bigotimes \pi \otimes  (\sigma \odiv \exists_x \pi) 
= \alpha \otimes \bigotimes \pi \otimes  \sigma_0
$$
and the result follows. For proving $1$ we used that $\langle \exists^\pi_x A, \sigma \rangle$ is a reachable configuration
and Lemma~\ref{l-mono2}, for $2$ we used that $\C$ is invertible and $\bigotimes \pi \leq \exists_x \pi$,
for $3$ we used the well-known equality $a \otimes ((a \otimes b) \odiv a) = a \otimes b$.
\qed
\end{proof}
...
$\langle A, \sigma \rangle \mapsto \langle B, \sigma' \rangle$ a reduction. 
...
Moreover, if  $\langle A, \tau \rangle \mapsto \langle B, \tau' \rangle$ and
$\langle A, \sigma \rangle \mapsto \langle B, \sigma' \rangle$ are
two reductions, then $\sigma' = \tau' \otimes (\sigma \odiv \tau)$.

\end{lemma}

The proof f the first item  simply consists in noting that if  $\langle A, \tau \rangle$ is reachable from 
 $\langle B, \mu \rangle$, then $\langle A, \sigma \rangle$ is reachable from
 $\langle \hbox{\tell}(\sigma \odiv \tau) \rightarrow \mathit{\ostop} \parallel B, \mu \rangle$.


\begin{lemma}
\label{riminor}
Let $\langle A, \tau \rangle \xrightarrow{\beta} \langle B, \tau' \rangle$ be a reduction
such that $\alpha \leq \beta$. 
%
...
for some $\alpha$ such that $\sigma = \tau \otimes \alpha \otimes \sigma''$, $\sigma \odiv \tau \leq \alpha$
and $\sigma' = \tau' \otimes \sigma''$ for some $\sigma''$.
\end{theorem}

\begin{proof}
We proceed by structural induction on the proof.
The result is straightforward for rules ${\bf LA1}$, ${\bf LA2}$, and ${\bf LR1}$ with 
$\alpha = \sigma \odiv \tau$ and $\sigma'' = \monid$, since $\C$ is invertible.

As for ${\bf LA2}$, note that $\sigma' = \sigma \leq c$ and that 
$\langle \hbox{\ask}(c) \rightarrow A, \tau \rangle \xrightarrow{\alpha} \langle A, \alpha \otimes \tau \rangle$ 
for any $\alpha \leq c\odiv \tau$, so that $\tau' = \alpha \otimes \tau$.
So, we just need to identify an $\alpha$ such that
$\sigma = \alpha \otimes \tau \otimes \sigma''$ for some $\sigma''$.
A possible witness for $\alpha$ is $c \odiv \tau$, since $\sigma \leq c$
implies $\sigma \odiv \tau \leq \alpha$.
%
Irregardless of $\alpha$, if $\sigma \leq \alpha \otimes \tau$ 
a possible witness for $\sigma''$ is $\sigma \odiv (\alpha \otimes \tau)$,
since the result would follow by invertibility. 
Again by invertibility, proving $\sigma \leq \alpha \otimes \tau$ 
corresponds to prove 
$(\sigma \odiv \tau) \otimes \tau \leq \alpha \otimes \tau$, which holds 
for $\alpha = c \odiv \tau$.

Let us  move to ${\bf LR2}$ and consider the configurations 
...
$\sigma = (\sigma \odiv \exists_x \pi) \otimes \exists_x \pi$ and
$\tau = (\tau \odiv \exists_x \pi) \otimes \exists_x \pi$
by Lemma~\ref{mono2}.
%
We have 
$$\langle \exists^\pi_x A, \sigma \rangle \mapsto
  \langle \exists^{\pi \rho}_x B, \sigma \otimes \exists_x \rho \rangle$$
  with $\sigma_0 = \sigma \odiv \exists_x \pi$ and $\rho = \sigma_1 \odiv (\bigotimes \pi  \otimes \sigma_0)$.
The premise tells us that $\langle A, \bigotimes \pi \otimes \sigma_0 \rangle$ is reachable and
$$\langle A, \bigotimes \pi \otimes \sigma_0 \rangle \mapsto
  \langle B, \sigma_1 \rangle$$
%
% such that $\sigma_1 =  (\sigma_1 \odiv ( \bigotimes \pi \otimes \sigma_0)) \otimes  \bigotimes \pi \otimes \sigma_0$
% by Lemma~\ref{mono}.
Now let us consider $\tau_0 = \tau \odiv \exists_x \pi$: clearly $\sigma_0 \leq \tau_0$,
so by induction hypothesis
$$\langle A, \bigotimes \pi \otimes \tau_0 \rangle  \xrightarrow{\beta}
  \langle B, \tau_1 \rangle$$  for some $\beta$ such that 
  $\bigotimes \pi \otimes \sigma_0 = \bigotimes \pi \otimes \tau_0 \otimes \beta \otimes \sigma_2$,
  $(\bigotimes \pi \otimes \sigma_0) \odiv (\bigotimes \pi \otimes \tau_0) \leq \beta$,
  and $\sigma_1 = \tau_1 \otimes \sigma_2$
  for some $\sigma_2$.
%Moreover, 
...
by invertibility, and by then it follows that $\sigma \odiv \tau \leq \beta$.
...
\section{On the equivalence between bisimulation semantics [TO BE DONE]}

\section{Concluding Remarks [TO BE DONE]}\label{sec:conclusion}
