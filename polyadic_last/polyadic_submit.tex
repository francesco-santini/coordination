...
The CCP language we present is a generalisation of what can be found in the literature (see also Sect.~\ref{sec:related}): first, it can naturally model bipolar (i.e., positive/negative) preferences within a single structure without joining two semirings together~\cite{posneg}, and secondly, the use of polyadic algebras can be exploited to model
...
An agent expressed in this language may perform operations  on variables (in particular, adding constraints) that are visible only to such an agent: for this reason, the \emph{hiding} operator needs to consider the effect of local steps with respect to the global store, seen by all the agents.
This paper is a continuation of \cite{festcatuscia} with a concurrent constraint language on top of its, and process-equivalence relations.


The work is organised as  follows: in Sect~\ref{sec:related} we summarise the most related work about CCP-based languages with the notion of local and global variables. Then, Sect.~\ref{sec:bg}  presents the necessary background on the algebraic structure needed to model polyadic constraints. Section~\ref{sec:newconstraint}
