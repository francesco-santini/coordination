...
%and we remark that such a $\xi$ is the same for all stores.
...
Consider ${\bf A1}$. 
%and $\langle \hbox{\tell}(c) \mapsto A, \tau \rangle \xrightarrow{} \langle A, c \otimes \tau \rangle$.
Thanks to Lemma~\ref{LA1}
the result follows with $\alpha = \1$,
since 
%$\sigma' = c \otimes \sigma$ and $\tau' = c \otimes \tau$ 
...
%the configurations 
%$\langle \exists^\pi_x A, \tau \rangle$  and
%$\langle \exists^\pi_x A, \sigma \rangle$
...
%since from these properties and cancellativeness 
%
%invertibility it follows that
...
% $a \leq b$ implies $a \odiv b \leq (a \otimes c) \odiv (b \otimes c)$,
...
$\alpha = \1$ and
%
we must show the equivalence of $\rho =  \tau_1 \odiv (\pi_0 \otimes \tau)$ and
$\chi = \sigma_1 \odiv (\pi_0 \otimes \sigma)$.
%
