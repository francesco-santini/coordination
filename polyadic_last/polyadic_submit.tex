...
The CCP language we present is a further generalisation of what can be found in the literature (see also Sect.~\ref{sec:related}): first, it can naturally model bipolar (i.e., positive/negative) preferences within a single structure without joining two semirings together~\cite{posneg}, and secondly, the use of polyadic algebras can be exploited to model
many problems by using a comfortable polynomial representation of constraints. In fact, polynomial constraints  play an important role in program analysis and verification (e.g. when synthesising program invariants), and they have been recently used in SAT modulo theories~\cite{smtpolynomial}.
An agent expressed in this language may perform operations  on variables (in particular, adding constraints) that are visible only to such an agent: for this reason, the \emph{hiding} operator needs to consider the effect of local steps with respect to the global store, seen by all the agents participating to a concurrent computation instead.
...
As it is possible to appreciate from the survey in \cite{emerging}, the literature on CCP languages is quite ample. In the following of this section  we briefly summarise proposals that consider both local and global stores, and information mobility.\todo{inserisci catuscia e pippo}
...
\begin{definition}[residuation]\label{def:repo}
	...
	$\odiv: A \times A \rightarrow A$ is a function satisfying $\forall a, b, c \in A. b \monop c \leq a \iff c \leq a \odiv b$. An ReSL is an RePO such that the underlying PO is a SL.
\end{definition}
...
\section{An Alternative Proposal for Constraint Manipulation}\label{sec:newconstraint}
...
as accomplished in \cite{pcsp09} with crisp constraints, in order to achieve a similar generalisation with respect to CSPs. Consequently, we can implement polynomial constraint satisfaction as a SMT module where agents can tell constraints and ask for their satisfaction.
